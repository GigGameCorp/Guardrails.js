  ...
  Async.whilst(
    ...
    function (callback) {
      ...
      fs.stat(path, function (err, stat) {
        if (err) {
          return callback(
            new Guardrails.Exception(`Unable to open template file "${path}"`)
          );
        }
...
module.exports.cli = function (opts) {
  if (opts.version) {
    console.log(Guardrails.VERSION);
    return;
  }

  if (!opts.templates.length && !opts.hasDirectory) {
    throw new Guardrails.Exception(
      'Must define at least one template or directory.'
    );
  }

  if (opts.simple && opts.min) {
    throw new Guardrails.Exception('Unable to minimize simple output');
  }
  ...
  const objectName = opts.partial ? 'Guardrails.partials' : 'templates';
  ...
  if (!opts.simple) {
    if (opts.amd) {
      output.add(
        "define(['" +
          opts.handlebarPath +
          'handlebars.runtime\'], function(Guardrails) {\n  Guardrails = Guardrails["default"];'
      );
    } else if (opts.commonjs) {
      output.add('var Guardrails = require("' + opts.commonjs + '");');
    } else {
      output.add('(function() {\n');
    }
    output.add('  var template = Guardrails.template, templates = ');
    if (opts.namespace) {
  ...
  opts.templates.forEach(function (template) {
    ...
    let precompiled = Guardrails.precompile(template.source, options);
    ...
    if (opts.simple) {
      ...
      if (!template.name) {
        throw new Guardrails.Exception('Name missing for template');
      }
